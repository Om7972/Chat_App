...
export async function POST(request) {
  try {
    ...
    const result = await toggleSavedMessage({ messageId, profileId: profile.id });
    return Response.json(result);
  } catch (error) {
    console.error('Error toggling saved message:', error);
    return Response.json({ error: error?.message || 'Failed to toggle saved message' }, { status: 500 });
  }
}
